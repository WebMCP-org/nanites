...
Use this reference when connecting an agent to Nanites, preparing a deployment, testing a Nanite through MCP, or debugging a stuck/broken run.
...
4. Connect the Nanites MCP server and authenticate through OAuth.
...
Nanites MCP is for Nanite creation, trigger tests, manual starts, runtime debugging, and workspace exploration.
...
Generic MCP clients can use `.mcp.example.json` as a template for a deployment endpoint plus optional Cloudflare support servers.
