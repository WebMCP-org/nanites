 ...
 * - Shared business enums that are also needed outside the DB package live in `@nanites/domain`.
...
export const accountRepositories = sqliteTable(
  "account_repositories",
  {
    ...
    permissionTier: text("permission_tier", { enum: REPOSITORY_PERMISSION_TIERS }),
    configSource: text("config_source", { enum: CONFIG_SOURCES }),
    configEnabled: integer("config_enabled", { mode: "boolean" }).notNull().default(true),
    configuredNaniteCount: integer("configured_nanite_count").notNull().default(0),
    mcpServerCount: integer("mcp_server_count").notNull().default(0),
    missingSoulDocumentCount: integer("missing_soul_document_count").notNull().default(0),
    missingSkillDocumentCount: integer("missing_skill_document_count").notNull().default(0),
    brokenPromptConfig: integer("broken_prompt_config", { mode: "boolean" })
      .notNull()
      .default(false),
    lastConfigFetchedAt: integer("last_config_fetched_at", { mode: "timestamp" }),
...
export const accountInstallationRepositoryMap = sqliteTable(
  "account_installation_repository_map",
  {
    githubInstallationId: integer("github_installation_id")
      .notNull()
      .references(() => accountInstallations.githubInstallationId, { onDelete: "cascade" }),
  ...
  (table) => [primaryKey({ columns: [table.githubInstallationId, table.githubRepositoryId] })],
);
