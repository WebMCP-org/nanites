...
export { parseSkillMarkdown } from "agents/skills";

export const LINKED_SKILLS_INDEX_PATH = "/.sigvelo/skills/index.json";
export const LINKED_SKILLS_PREFIX = "/.sigvelo/skills";
export const LINKED_SKILLS_INDEX_VERSION = 1;
...
export type LinkedSkillResourceDescriptor = {
  path: string;
...
export type InstalledLinkedSkill = {
  name: string;
  description: string;
...
export type InstalledLinkedSkillSource = {
  sourceUrl: string;
  owner: string;
  repo: string;
  ref: string;
...
export function validateLinkedSkillRelativePath(path: string): boolean {
  return (
    path.length > 0 &&
    !path.startsWith("/") &&
    !path.includes("\0") &&
...
export function parseGitHubSkillSource(input: string): GitHubSkillSource {
...
export async function ensureLinkedSkillUrls(input: {
  workspace: SkillLinkWorkspace;
...
export function workspaceSkillLinks(
  workspace: SkillLinkWorkspace,
