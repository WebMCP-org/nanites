...
import type { ManagerReplyPublication, SigveloChatIngress } from "#/backend/chat-ingress.ts";
import type { HandleManagerChatMessageInput } from "#/backend/nanites/host.ts";
import { MCP_SCOPES } from "#/shared/constants/mcp.ts";
import { buildNaniteManagerKey } from "#/shared/nanites.ts";

const SIGVELO_MCP_SERVER_NAME_PREFIX = "sigvelo-github-manager-chat";
...
export type ManagerGitHubMessageAcceptance = {
  accepted: boolean;
...
export type ManagerGitHubReplyStatus =
  | {
      status: "ready";
      text: string;
    }
...
export type ManagerGitHubReplyPublicationInput = Omit<
...
export class SigveloManagerConversationAgent extends Think<Env, ManagerConversationState> {
  ...
  async connectBrowserInstallation(
    ...
    const props = createSigveloMcpPropsFromBrowser(input);
    await this.ensureSigveloMcpConnected(props, requireSelectedGitHubAccount(input.accountLogin));
    return { connected: true };
  }

  async connectSigveloMcp(input: HandleManagerChatMessageInput): Promise<void> {
    const props = createSigveloMcpProps(input);
    await this.ensureSigveloMcpConnected(props, getRepositoryOwner(input.surface.repository));
  }

  private async ensureSigveloMcpConnected(props: SigveloMcpAuthProps, accountLogin: string) {
    const sigveloServerName = sigveloMcpServerName(props);
    await this.removeFailedMcpServer(sigveloServerName);
    await this.addMcpServer(sigveloServerName, this.env.SigveloMcpAgent, { props });
    await this.ensureGitHubMcpConnected(props, accountLogin);
  }

  private async removeFailedMcpServer(serverName: string): Promise<void> {
    const failedServerIds = this.mcp
      .listServers()
      .filter(
        (server) =>
          server.name === serverName &&
          this.mcp.mcpConnections[server.id]?.connectionState === "failed",
      )
      .map((server) => server.id);

    await Promise.all(failedServerIds.map((serverId) => this.mcp.removeServer(serverId)));
  }
  ...
  private async ensureGitHubMcpConnected(props: SigveloMcpAuthProps, accountLogin: string) {
    ...
    const repositoryNames = repositories.map((repository) => repository.full_name).sort();
    this.setState({
      status: "connected",
  ...
  private formatInstallationContext(): string {
    ...
    return [
      ...
      ...context.repositoryNames.map((repository) => `- ${repository}`),
      "",
  ...
  async answerGitHubMessage(
    ...
    publication?: ManagerGitHubReplyPublicationInput,
  ): Promise<ManagerGitHubMessageAcceptance> {
    await this.connectSigveloMcp(input);
    ...
    const submission = await this.submitMessages([message], {
      ...
      metadata: {
        ...
        repository: input.surface.repository,
  ...
  private createGitToolProvider() {
    return gitToolsWithGitHubInstallationAuth(this.workspace, {
      getAllowedRepositories: () => {
        ...
        return state.status === "connected" ? state.repositoryNames : [];
      },
  ...
  private async issueGitToolToken(repository: string): Promise<string | null> {
    ...
    const scopedToken = await issueScopedGitHubInstallationToken({
      env: this.env,
      installationId: githubInstallationIdSchema.parse(state.githubInstallationId),
...
function createSigveloMcpProps(input: HandleManagerChatMessageInput): SigveloMcpAuthProps {
  return createSigveloMcpPropsFromBrowser({
    managerName: buildNaniteManagerKey(input.installationId),
    githubInstallationId: input.installationId,
    accountLogin: getRepositoryOwner(input.surface.repository),
    actor: {
      id: Number(input.author.userId),
      login: input.author.userName,
    },
  });
}

function createSigveloMcpPropsFromBrowser(input: ManagerBrowserSessionInput): SigveloMcpAuthProps {
  requireSelectedGitHubAccount(input.accountLogin);
  const installationId = githubInstallationIdSchema.parse(input.githubInstallationId);
  if (input.managerName !== buildNaniteManagerKey(installationId)) {
    throw new Error("Manager conversation installation does not match the selected manager.");
  }

  const props = {
    authKind: "mcp",
    githubUserId: githubUserIdSchema.parse(input.actor.id),
...
function requireSelectedGitHubAccount(accountLogin: string): string {
  ...
  if (!trimmed) {
    throw new Error("Manager conversation requires a selected GitHub account.");
  }

  return trimmed;
}

function getRepositoryOwner(repository: string): string {
  const [owner] = repository.split("/", 1);
  return requireSelectedGitHubAccount(owner ?? "");
}

function sigveloMcpServerName(props: SigveloMcpAuthProps): string {
  return [SIGVELO_MCP_SERVER_NAME_PREFIX, props.githubInstallationId, props.githubUserId].join("-");
}
...
export function buildManagerSystemPrompt(): string {
  return [
    ...
    "Use Sigvelo MCP tools for control-plane work: inspect Nanites, create or update Nanites, deprovision Nanites, start manual runs, cancel runs, and inspect Nanite workspaces.",
...
export function formatNanitesAuthoringSources(): string {
  return [
...
export function toManagerConversationMessage(input: HandleManagerChatMessageInput): UIMessage {
  return {
    ...
    parts: [
      {
        ...
        text: [
          ...
          `Repository: ${input.surface.repository}`,
          `Thread: ${input.surface.threadType} #${input.surface.prNumber}`,
          "",
          input.text,
        ].join("\n"),
      },
