...
- `../agents/SigveloNaniteAgent.ts` owns the stable Think agent runtime: durable chat history,
  workspace access, execution tools, GitHub-aware git auth, and Workflow-owned structured output.
- `../agents/SigveloManagerConversationAgent.ts` owns manager chat and Think messenger ingress.
...
- `triggers.ts` owns generated inbound trigger execution through Worker Loader.
- `tool-output.ts` owns current-run output artifacts and inline output budgeting.

The manager `testNaniteTrigger` callable is the authoring-agent acceptance loop. It validates a raw
GitHub webhook-shaped test event, runs the Nanite's generated inbound trigger, dispatches the real
ThinkWorkflow-backed Nanite Run, waits for terminal structured output, and returns any
`agentFeedback` the Nanite provides to the coding agent. MCP exposes that same manager primitive as
...
access for the deployment installation while it investigates and delegates, but each Nanite runtime
