...
import type {
  ...
  SigveloChatIngress,
} from "#/backend/nanites/chat-ingress.ts";
import {
...
export class SigveloManagerConversationAgent extends Think<Env, ManagerConversationState> {
  ...
  async connectSigveloMcp(input: HandleManagerChatMessageInput): Promise<void> {
    ...
    await this.ensureSigveloMcpConnected(props, getRepositoryOwner(input.surface.raw.repository));
  }
...
function createSigveloMcpProps(input: HandleManagerChatMessageInput): SigveloMcpAuthProps {
  ...
  if (!Number.isInteger(installationId) || installationId <= 0) {
    throw new Error("Manager conversation requires a valid GitHub installation id.");
  }
