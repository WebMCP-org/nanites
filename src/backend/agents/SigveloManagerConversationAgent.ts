...
function buildManagerSystemPrompt(): string {
  return [
    ...
    "Use built-in workspace tools for repository file review: read, list, grep, find, write, edit, delete, and execute with state.* plus git.* for clone, fetch, branch, commit, and push work. Clone and fetch are runtime-enforced with depth: 1; git.pull is disabled.",
    ...
    `Before creating or updating Nanites, try to refresh the Nanites authoring repo ${NANITES_AUTHORING_REPOSITORY}: inside execute, use git.clone to clone ${NANITES_AUTHORING_REPOSITORY_URL} into ${NANITES_AUTHORING_CHECKOUT_DIR} if missing, otherwise use git.fetch for the latest default branch there.`,
...
function formatNanitesAuthoringSources(): string {
  return [
    ...
    `- If the checkout already exists, call git.fetch inside execute before reading Nanite authoring files.`,
