...
- The default template has no runtime `vars`; setup infers the Worker script name from the
  `workers.dev` hostname on the public deploy path.
...
{
  "scripts": {
    "deploy": "vp build && vp exec wrangler deploy ... && vp run db:migrate:remote",
    "deploy:validate": "vp build && vp exec wrangler deploy ... --dry-run"
  }
}
```
