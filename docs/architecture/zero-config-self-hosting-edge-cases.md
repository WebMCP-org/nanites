...
| User starts setup from a custom domain                                                 | Verify the Cloudflare account owns the custom domain route and knows the Worker script name.                                                                                             | Supported when `NANITES_CLOUDFLARE_SCRIPT_NAME` is configured. `workers.dev` can infer the script name from the hostname.                               | Ensure the deploy template always carries the script-name setup hint.                                                                                        |
...
| Setup claim expires before GitHub App creation finishes                                | Reject the GitHub step and require reconnecting Cloudflare. Do not ask for manual secrets.                                                                                               | Implemented. Setup claim TTL is 30 minutes.                                                                                                             | UI should say the setup session expired and reconnecting Cloudflare is the fix.                                                                              |
...
| Runtime model access fails after setup                                                 | Setup may complete but the app cannot run Nanite work.                                                                                                                                   | Default model now uses a Cloudflare-hosted function-calling model, but setup does not run an AI smoke.                                                  | Add final readiness smoke: one short non-tool model call and one tool-capable Nanite run.                                                                    |
...
- Deploy to Cloudflare lets users customize Worker names. The template pins both `name` and
  `NANITES_CLOUDFLARE_SCRIPT_NAME`; if Cloudflare updates one but not the other, workers.dev
  ownership verification can reject the correct account. Treat a stale script-name hint as a setup
  diagnostic.
